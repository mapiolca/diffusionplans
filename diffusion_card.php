...
if ($action == 'create') {
	...
	if (isModEnabled('project') && is_object($formproject)) {
		...
		print img_picto('', 'project', 'class="pictofixedwidth"').$formproject->select_projects(($soc->id > 0 ? $soc->id : -1), $fk_project, 'fk_project', 0, 0, 1, 1, 0, 0, 0, '', 1, 0, 'maxwidth500 widthcentpercentminusxx');
...
*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id."&src_object_type=diffusion";
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('diffusion:Diffusion', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		/*
