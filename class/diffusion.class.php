...
/**
 * \file        class/diffusion.class.php
 * \ingroup     diffusionplans
 * \brief       This file is a CRUD class file for Diffusion (Create/Read/Update/Delete)
 */
 ...
 * Class for Diffusion
 */
class Diffusion extends CommonObject
{
	/**
	...
	/**
	 * @var string 	String with name of icon for diffusion. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'diffusion@diffusionplans' if picto is file 'img/object_diffusion.png'.
	 */
	...
	public $fields = array(
		...
		"label" => array("type" => "varchar(255)", "label" => "Label", "enabled" => "1", 'position' => 30, 'notnull' => 0, "visible" => "1", "searchall" => "1", "css" => "minwidth300", "cssview" => "wordbreak", "help" => "label_help", "validate" => "1",),
	...
	//  */
	//public $class_element_line = 'DiffusionContact';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	//protected $childtables = array('mychildtable' => array('name'=>'Diffusion', 'fk_element'=>'fk_diffusion'));
	 ...
	 */
		...
		/*if ($user->hasRight('diffusionplans', 'diffusion', 'read')) {
	...
	public function create(User $user, $notrigger = 0)
	{
		...
		global $langs, $user, $conf, $object, $db;
	    ...
	    if ($resql)
	        {			
	            if (! $notrigger)
	            {
	                ...
	                $result = $interface->run_triggers('DIFFUSION_INSERT', $this, $user ,$langs, $conf);
	                if ($result < 0) {
	...
	{	
        ...
        dol_syslog("DiffusionContact::deleteAll sql=".$sql);
        //var_dump($sql);
        ...
        if ($resql)
        {
            ...
            include_once DOL_DOCUMENT_ROOT . '/core/class/interfaces.class.php';
            $interface=new Interfaces($this->db);
            $result=$interface->run_triggers('DIFFUSIONCONTACT_DELETEALL',$this,$user,$langs,$conf);
        ...
        else
        {
            ...
            dol_syslog("DiffusionContact::deleteAll ".$this->error, LOG_ERR);
            return -1;
        }
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusionplans', 'diffusion', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusionplans', 'diffusion_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'diffusion/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'diffusion/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'diffusion/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'diffusion/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->diffusionplans->dir_output.'/diffusion/'.$oldref;
				$dirdest = $conf->diffusionplans->dir_output.'/diffusion/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->diffusionplans->dir_output.'/diffusion/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'DIFFUSION_DIFFUSION_UNVALIDATE');
	}
	...
	public function setSent($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_SENT, $notrigger, 'DIFFUSION_DIFFUSION_SENT');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'DIFFUSION_DIFFUSION_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'DIFFUSION_DIFFUSION_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDiffusion")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Diffusion").'</u>';
	...
	public function getLinesArray()
	{
		...
		$objectline = new DiffusionLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_diffusion:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('DIFFUSIONPLANS_DIFFUSION_ADDON')) {
			$conf->global->DIFFUSIONPLANS_DIFFUSION_ADDON = 'mod_diffusion_standard';
		}

		if (getDolGlobalString('DIFFUSIONPLANS_DIFFUSION_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('DIFFUSIONPLANS_DIFFUSION_ADDON').".php";
			$classname = getDolGlobalString('DIFFUSIONPLANS_DIFFUSION_ADDON');
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefDiffusion $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_diffusion';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (getDolGlobalString('DIFFUSION_ADDON_PDF')) {
				$modele = getDolGlobalString('DIFFUSION_ADDON_PDF');
			}
		}
...
/**
 * Class DiffusionLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DiffusionLine extends CommonObjectLine
{
	// To complete with content of an object DiffusionLine
	// We should have a field rowid, fk_diffusion and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	//public $parent_element = 'diffusion';		// Example: '' or 'diffusion'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	//public $fk_parent_attribute = 'fk_diffusion';	// Example: '' or 'fk_diffusion'
