...
/**
 * \file        class/diffusioncontact.class.php
 * \ingroup     diffusionplans
 * \brief       This file is a CRUD class file for DiffusionContact (Create/Read/Update/Delete)
 */
...
/**
 * Class for DiffusionContact
 */
class DiffusionContact extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'diffusioncontact';
	 ...
	 */
	//public $element_for_permission = 'diffusionplans';

	/**
	 * @var string 	String with name of icon for diffusioncontact. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'diffusioncontact@diffusionplans' if picto is file 'img/object_diffusioncontact.png'.
	 */
	...
	//  */
	public $fk_element = 'fk_diffusion';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'DiffusionContactline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'DiffusionContact', 'fk_element'=>'fk_diffusioncontact'));
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('diffusionplans', 'diffusioncontact', 'read')) {
	...
	public function create($notrigger = 0)
	{
		...
		global $langs, $user, $conf, $object, $contactid;

		$this->db->begin();

		$sql = "INSERT INTO ".MAIN_DB_PREFIX."diffusionplans_diffusioncontact";
        $sql.= " (`fk_diffusion`, `fk_contact`, `contact_source`, `mail_status`, `letter_status`, `hand_status`)";
        $sql.= " VALUES (".$object->id.",";
        $sql.= " '".$contactid."', ";
        $sql.= " '".GETPOST("source", 'aZ09')."', ";
        $sql.= " 0,";
        $sql.= " 0,";
        $sql.= " 0";
        $sql.= ')';
        ...
        dol_syslog("DiffusionContact::insert sql=".$sql);

	    $resql = $this->db->query($sql);
	    //var_dump("Contact ID : ".$resql);
	 ...
	 */
	public function deleteLine($notrigger = 0)
	{
		//if ($this->status < 0) {
		...
		global $conf, $user, $langs, $object, $contactid;
        ...
        dol_syslog("DiffusionContact::deleteLine sql=".$sql);
        //var_dump($sql);
        ...
        if ($resql)
        {
            ...
            include_once DOL_DOCUMENT_ROOT . '/core/class/interfaces.class.php';
            $interface=new Interfaces($this->db);
            $result=$interface->run_triggers('DIFFUSIONCONTACT_DELETELINE',$this,$user,$langs,$conf);
        ...
        else
        {
            ...
            dol_syslog("DiffusionContact::deleteLine ".$this->error, LOG_ERR);
            return -1;
        }
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusionplans', 'diffusioncontact', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusionplans', 'diffusioncontact_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'diffusioncontact/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'diffusioncontact/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'diffusioncontact/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'diffusioncontact/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->diffusionplans->dir_output.'/diffusioncontact/'.$oldref;
				$dirdest = $conf->diffusionplans->dir_output.'/diffusioncontact/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->diffusionplans->dir_output.'/diffusioncontact/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusionplans','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('diffusionplans','diffusionplans_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'DIFFUSIONPLANS_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'DIFFUSIONPLANS_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'DIFFUSIONPLANS_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDiffusionContact")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DiffusionContact").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/diffusionplans/diffusioncontact_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowDiffusionContact");
	...
	public function getLinesArray()
	{
		...
		$objectline = new DiffusionContactLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_diffusioncontact:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('DIFFUSIONPLANS_MYOBJECT_ADDON')) {
			$conf->global->DIFFUSIONPLANS_MYOBJECT_ADDON = 'mod_diffusioncontact_standard';
		}

		if (getDolGlobalString('DIFFUSIONPLANS_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('DIFFUSIONPLANS_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('DIFFUSIONPLANS_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/diffusionplans/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefDiffusionContact $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_diffusioncontact';
		...
		$modelpath = "core/modules/diffusionplans/doc/";
...
/**
 * Class DiffusionContactLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DiffusionContactLine extends CommonObjectLine
{
	// To complete with content of an object DiffusionContactLine
	// We should have a field rowid, fk_diffusioncontact and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'diffusioncontact'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_diffusioncontact'
