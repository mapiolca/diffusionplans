...
/**
 *	\file       htdocs/core/modules/diffusionplans/doc/doc_generic_diffusion_odt.modules.php
 *	\ingroup    diffusionplans
 *	\brief      File of class to build ODT documents for diffusions
 */
 ...
 */
class doc_generic_diffusion_odt extends ModelePDFDiffusion
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'DIFFUSIONPLANS_MYOBJECT_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$texte .= '<input type="hidden" name="param1" value="DIFFUSIONPLANS_MYOBJECT_ADDON_PDF_ODT_PATH">';
		$texte .= '<table class="nobordernopadding centpercent">';

		// List of directories area
		$texte .= '<tr><td>';
		$texttitle = $langs->trans("ListOfDirectories");
		$listofdir = explode(',', preg_replace('/[\r\n]+/', ',', trim(getDolGlobalString('DIFFUSIONPLANS_MYOBJECT_ADDON_PDF_ODT_PATH'))));
		$listoffiles = array();
		...
		if (!getDolGlobalString('MAIN_NO_MULTIDIR_FOR_ODT')) {
			...
			$texte .= getDolGlobalString('DIFFUSIONPLANS_MYOBJECT_ADDON_PDF_ODT_PATH');
			$texte .= '</textarea>';
			...
			$texte .= '<br>';
			$texte .= '<input type="hidden" name="value1" value="DIFFUSIONPLANS_MYOBJECT_ADDON_PDF_ODT_PATH">';
		}

		// Scan directories
		$nbofiles = count($listoffiles);
		if (getDolGlobalString('DIFFUSIONPLANS_MYOBJECT_ADDON_PDF_ODT_PATH')) {
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=diffusionplans_diffusion/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=DIFFUSIONPLANS_MYOBJECT_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		$texte .= '<input type="hidden" value="DIFFUSIONPLANS_MYOBJECT_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->diffusionplans->dir_output) {
			// If $object is id instead of object
			if (!is_object($object)) {
				$id = $object;
				$object = new Diffusion($this->db);
			...
			$dir = $conf->diffusionplans->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->diffusionplans->dir_temp);
				if (!is_writable($conf->diffusionplans->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->diffusionplans->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'DIFFUSIONPLANS_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->diffusionplans->dir_temp,
