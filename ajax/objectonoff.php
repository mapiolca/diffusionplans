...
require_once '../class/diffusioncontact.class.php';
...
if (($action == 'set') && !empty($id)) {	// Test on permission already done in header according to object and field.
	...
	$diffusioncontactupdate = new DiffusionContact($db);

	$update = $diffusioncontactupdate->update($user, $field, $value, $object->table_element, $id);
	var_dump($update);
