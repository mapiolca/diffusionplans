...
require_once DOL_DOCUMENT_ROOT.'/custom/diffusionplans/lib/diffusionplans.lib.php';
require_once DOL_DOCUMENT_ROOT.'/custom/diffusionplans/lib/diffusionplans.lib.php';
require_once DOL_DOCUMENT_ROOT.'/custom/diffusionplans/lib/ajax.lib.php';
...
if (!isset($permission) && $user->hasRight($module, 'creer')) {
...
$arrayfields = array(
	...
	'mail_status' 		=> array('label' => $langs->trans("methodMail"), 'checked' => 1),
	'letter_status' 		=> array('label' => $langs->trans("methodLetter"), 'checked' => 1),
	'hand_status' 		=> array('label' => $langs->trans("methodHand"), 'checked' => 1),
...
print_liste_field_titre($arrayfields['mail_status']['label'], $_SERVER["PHP_SELF"], "mail_status", "", $param, "", $sortfield, $sortorder);
print_liste_field_titre($arrayfields['letter_status']['label'], $_SERVER["PHP_SELF"], "letter_status", "", $param, "", $sortfield, $sortorder);
print_liste_field_titre($arrayfields['hand_status']['label'], $_SERVER["PHP_SELF"], "hand_status", "", $param, "", $sortfield, $sortorder);
...
foreach ($list as $entry) {
	...
	$line_diffusioncontact = $diffusioncontactstatic->fetch($entry);
	$diffusioncontactstatic->id = $line_diffusioncontact['id'];
	$diffusioncontactstatic->element = $line_diffusioncontact['element'];
	$diffusioncontactstatic->table_element = $line_diffusioncontact['table_element'];
	$diffusioncontactstatic->diffusion = $line_diffusioncontact['diffusion'];
	$diffusioncontactstatic->mail_status = $line_diffusioncontact['mail_status'];
	$diffusioncontactstatic->letter_status = $line_diffusioncontact['letter_status'];
	$diffusioncontactstatic->hand_status = $line_diffusioncontact['hand_status'];
	...
	print '<td class="norap" data-nature="' . dol_escape_htmltag($entry->nature) . '"><span class="opacitymedium">'.dol_escape_htmltag($entry->nature_html).'</span></td>';
	print '<td class="norap" data-type_id="' . ((int) $entry->type_id) . '" data-type="' . dol_escape_htmltag($entry->type) . '">'.dol_escape_htmltag($entry->type).'</td>';
	...
	print '</td>';
		...
		//print '<td class="tdoverflowmax200 center"></td>';
	...
	if ($permission && $object->status == $object::STATUS_DRAFT) {
		...
		$href .= '&source='.$contact['source'];
		if ($contact['source'] == 'internal') {
			$href .= '&userid='.((int) $entry->contact_id);
		} elseif ($contact['source'] == 'external') {
			$href .= '&contactid='.((int) $entry->contact_id);
		}
