...
/**
 * \file    diffusionplans/admin/setup.php
 * \ingroup diffusionplans
 * \brief   DiffusionPlans setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "diffusionplans@diffusionplans"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('diffusionplanssetup', 'globalsetup'));
...
$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM1');
$item->fieldAttr['placeholder'] = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a Thirdparty
$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('DIFFUSIONPLANS_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('DIFFUSIONPLANS_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('DIFFUSIONPLANS_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('DIFFUSIONPLANS_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('DIFFUSIONPLANS_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('DIFFUSIONPLANS_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('DIFFUSIONPLANS_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf DIFFUSIONPLANS_MYPARAM10
$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('DIFFUSIONPLANS_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=diffusionplans-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DIFFUSIONPLANS_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'DIFFUSIONPLANS_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "DiffusionPlansSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-diffusionplans page-admin');
...
$head = diffusionplansAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "diffusionplans@diffusionplans");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DiffusionPlansSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DIFFUSIONPLANS_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DIFFUSIONPLANS_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
